...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(logOutReceived)
                                                 name:@"LogOut" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(loggedIn)
                                                 name:@"LoggedIn"
                                               object:nil];
    NSNumber *loggedIn = [[NSUserDefaults standardUserDefaults]
                          objectForKey:@"LoggedIn"];
    ...
    NSArray *horizontalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[B]-0-|"
                                                                             options:0
                                                                             metrics:nil
                                                                               views:dictionary];
...
- (void)loggedIn {
    ...
    [UIView animateWithDuration:0.33 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        activeController.view.layer.transform = CATransform3DMakeTranslation(0,
...
- (void)logOutReceived {
    ...
    viewController = [storyboard instantiateViewControllerWithIdentifier:@"ViewController"];
    viewController.view.frame = self.view.frame;
    viewController.view.layer.transform = CATransform3DMakeTranslation(0, viewController.view.frame.size.height,
                                                                       0);
    [self.view addSubview:viewController.view];
    
    
    [UIView animateWithDuration:0.66 delay:0 usingSpringWithDamping:0.85 initialSpringVelocity:0.5
