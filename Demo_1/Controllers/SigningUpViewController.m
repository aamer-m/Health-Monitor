...
@interface SigningUpViewController()<UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate>
...
- (void)adjustHeight:(BOOL)show forNotification:(NSNotification *) aNotification {
    ...
    CGFloat changeInHeight = (height + 20) * (show ? 1: -1);
    [UIScrollView animateWithDuration:animationDuration animations:^{
        self.countryBottomConstraint.constant += changeInHeight;
    }];
    ...
    if (!CGRectContainsPoint(aRect,
        ...
        CGPoint scrollPoint = CGPointMake(0.0, [self activeField].frame.origin.y - height - 30);
...
- (IBAction)createAccountTapped:(id)sender {
    ...
    if ([self.accountInformation isFormAcceptable:&error]) {
        [self performSegueWithIdentifier:@"CreateAccountLogin" sender:sender];
    }
    else {
